...
export const InterestTable = ({ amount, interest, date }: any) => {
  const pagos = Array(6).fill(0);
  let now = dayjs(date);

  for (let i = 0; i < pagos.length; i++) {
    pagos[i] = (amount * interest) / 6;
  }
  let amountInterest = amount * 1.2;
  ...
  return (
    <>
      ...
      <Text h3> Total: ${amount * 1.2}</Text>
      <Text h3> Ganancias: ${amount * 1.2 - amount}</Text>
    </>
  );
};
